  ...
  render() {
    ...
    return (
      <Grid className="insights-dashboard">
        ...
        <Col className="insight-card" xs={6} md={4}>
          <Thumbnail onClick={() => {window.location=`/s/${getSchoolId()}/networks/wf/events/insights?classroom_id=${getClassroomId()}&currentDate=${lastMonth}&endDate=${today}&visualization=studentSummary&currentEntityType=child&currentEntityId=${this.props.defaultEntity.id}`}} className="insight-thumbnail" src={`https://sensei-server.herokuapp.com/assets/studentSummary.png`} alt="242x200">
        ...
        <Col className="insight-card" xs={6} md={4}>
          <Thumbnail onClick={() => {window.location=`/s/${getSchoolId()}/networks/wf/events/insights?classroom_id=${getClassroomId()}&currentDate=${lastMonth}&endDate=${today}&visualization=unitSummary&currentEntityType=child&currentEntityId=${this.props.defaultEntity.id}`}} className="insight-thumbnail" src={`https://sensei-server.herokuapp.com/assets/unitSummary.png`} alt="242x200">
