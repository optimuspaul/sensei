...
export default function socialGraph() {
  ...
  let updateChart = (event) => {
    ...
    let currentEntity = storeEntities.children[currentEntityId];
    let currentEntityName = currentEntity ? currentEntity.displayName : "Unknown";
    ...
    let graphData = _.reduce(data.entities, (current, val, index) => {
      ...
      let name = entity ? entity.displayName : `${val[0]}-${val[1]}`;
      let node = {id: `${val[0]}-${val[1]}`, group: 1, label: name};
      ...
      return current;
     }, {nodes:[{id: `${currentEntityType}-${currentEntityId}`, group: 1, label: currentEntityName}], links: []})
    ...
    let refresh = () => {
      ...
      let ticked = () => {
        link
            .attr("x1", (d) => { debugger; return d.source.x; })
