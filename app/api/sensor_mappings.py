...
def create_sensor_mapping():
    ...
    for mapping in map_data:
        ...
        classroom_id = mapping.get('classroom_id')
        now = datetime.datetime.now()
        mappings_to_end = SensorMapping.query.filter_by(classroom_id=classroom_id,sensor_id=sensor_id,end_time=None).all()
        mappings_to_end.extend(SensorMapping.query.filter_by(classroom_id=classroom_id,entity_id=entity_id,entity_type=entity_type,end_time=None))
